...
class Conditional
{
    ...
    foreach ($fields as $field) {
      ...
      if ($type === 'checkbox') {
        if ($value === '0' && $meta_value) {
          return false;
        } else if ($value === '1' && !$meta_value) {
          return false;
        }
      } else if ($operator === '!=' && $meta_value === $value) {
        return false;
      } else if ($operator === '==' && $meta_value !== $value) {
        return false;
      }
  ...
  public function hide_or_show_content($block_content, $block) {
    ...
    $fields = isset($block['attrs']['fields']) ? $block['attrs']['fields'] : [];
    $showOnPostTypes = isset($block['attrs']['showOnPostTypes']) ? $block['attrs']['showOnPostTypes'] : [];
    $showOnTaxonomies = isset($block['attrs']['showOnTaxonomies']) ? $block['attrs']['showOnTaxonomies'] : [];
